...
Feature: Student Activity Routes
  Scenario: Receive a 200 status when go to PUT studentactivity with valid cookie and valid json body
    ...
    #Getting 200 currently - since it's create if not currently created and studentactivity id exists, it correctly doesn't implode? Swagger also doesn't call for any body
  Scenario: Receive a 200 status when go to PUT studentactivity with valid cookie and empty json body
    Given I get a student cookie and receive status '200'
    Given I PUT '{}' into ':3000/activity/d3e3c2d5-cf43-4f63-924f-3ec7a125a334/studentactivity' and receive status '200'
